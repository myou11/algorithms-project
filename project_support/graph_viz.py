...
def all_graph_files(directory_name=None):
    if directory_name is None:
        directory_name = ''
    elif not directory_name.endswith('/'):
        directory_name += '/'
    import re
    def graph_number(graph_file_name):
        return int(re.findall('graph(\d+)', graph_file_name)[0])
    import glob
    return list(sorted(glob.glob(directory_name + 'graph*.txt'), key=graph_number))
